...
- **Button legacy desalinhado do design system** (descoberto na Fase 2.5): `src/components/Button.tsx` usa tokens antigos (champagne/obsidian) e anti-patterns proibidos pelo `design.md` (`hover:scale-`, `transition-all`). Foi reusado nas Fases 2.1-2.4 sem detecção (grep de anti-patterns só varre arquivos novos da fase, não dentro de componentes reusados). Risco: divergência visual entre rotas que usam `Button` vs componentes novos da Fase 2.5+ que usam `button` cru. Conserto: refatorar `Button.tsx` pra tokens atuais (paper/ink/accent) + remover `hover:scale-`/`transition-all` + adicionar testes de regressão visual ou pelo menos auditoria estática que cubra `src/components/*`. Pickup: sub-fase 2.7 dedicada OU dentro da Fase 6 (que vai criar UI de assinatura usando o `Button`). Não bloqueante pra MVP.
